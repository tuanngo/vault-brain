...
def verify(base_source_root: Path, base_package: str):
    ...
    if not base_package_dir.is_dir():
        print(
            f"ERROR: Base package directory does not exist: {base_package_dir}",
            file=sys.stderr,
        )
        return 2
    ...
    for package_dir in package_dirs:
        expected_pkg = expected_package_name(
            base_package, package_dir, base_package_dir
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Verify @NullMarked package-info.java usage"
    )
